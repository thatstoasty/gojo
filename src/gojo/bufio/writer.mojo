...
struct Writer[W: MovableWriter, //](Sized):
    ...
    fn write_bytes(inout self, bytes: Span[Byte, _]) -> None:
        ...
        while len(bytes[start:end]) > self.available() and not self.err:
            bytes_written = 0
            bytes_to_write = bytes[start:end]
            # Large write, empty buffer. Write directly from bytes to avoid copy.
            if self.buffered() == 0:
    ...
    fn read_from[R: io.Reader](inout self, inout reader: R) raises -> Int:
        ...
        while True:
            ...
            while nr < MAX_CONSECUTIVE_EMPTY_READS:
                ...
                try:
                    bytes_read = reader.read(self.buf)
                except e:
                    err = e
                    break
