...
struct Scanner[R: io.Reader, //, split: SplitFunction = scan_lines]():
    ...
    fn current_token_as_bytes(ref [_]self) -> Span[Byte, __origin_of(self)]:
        """Returns the most recent token generated by a call to `Scanner.scan`."""
        return Span[Byte, __origin_of(self)](unsafe_ptr=self.token.unsafe_ptr(), len=self.token.size)

    fn current_token_as_string_slice(ref [_]self) -> StringSlice[__origin_of(self)]:
        """Returns the most recent token generated by a call to `Scanner.scan`."""
        return StringSlice[__origin_of(self)](unsafe_from_utf8_ptr=self.token.unsafe_ptr(), len=self.token.size)
        ...
        while True:
            ...
            if (self.end > self.start) or self.err:
                ...
                if self.err:
                    at_eof = True
                try:
                    advance, token = split(self.as_bytes()[self.start : self.end], at_eof)
                except e:
                    self.set_err(e)
                    return False
...
alias SplitFunction = fn (data: Span[Byte], at_eof: Bool) raises -> (Int, List[Byte, True])
...
###### split functions ######
fn scan_bytes(data: Span[Byte], at_eof: Bool) raises -> (Int, List[Byte, True]):
    ...
    if at_eof and len(data) == 0:
        return 0, List[Byte, True]()

    return 1, List[Byte, True](data[0:1])


fn scan_runes(data: Span[Byte], at_eof: Bool) raises -> (Int, List[Byte, True]):
    ...
    lhs = (((UnsafePointer[Scalar[DType.uint8]].load(data.unsafe_ptr()) >> 7) == 0) * 1).cast[DType.uint8]()
    rhs = count_leading_zeros(~UnsafePointer[Scalar[DType.uint8]].load(data.unsafe_ptr()))
    char_length = int(lhs + rhs)

    # Copy N bytes into new pointer and construct List.
    sp = UnsafePointer[Byte].alloc(char_length)
    parallel_memcpy(sp, data.unsafe_ptr(), char_length)
    result = List[Byte, True](unsafe_pointer=sp, size=char_length, capacity=char_length)

    return char_length, result


fn drop_carriage_return(data: Span[Byte]) -> List[Byte, True]:
...
fn scan_lines(data: Span[Byte], at_eof: Bool) raises -> (Int, List[Byte, True]):
...
fn scan_words(data: Span[Byte], at_eof: Bool) raises -> (Int, List[Byte, True]):
    ...
    while i < len(data):
        width = len(data[i])
        if is_space(data[i]):
            return i + width, List[Byte, True](data[start:i])

        i += width

    # If we're at EOF, we have a final, non-empty, non-terminated word. Return it.
    if at_eof and len(data) > start:
        return len(data), List[Byte, True](data[start:])

    # Request more data.
    return start, List[Byte, True]()
