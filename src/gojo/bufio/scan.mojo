...
struct Scanner[R: io.Reader, //, split: SplitFunction = scan_lines]():
    ...
    fn current_token_as_bytes_span(ref [_]self) -> Span[UInt8, __lifetime_of(self)]:
        """Returns the most recent token generated by a call to `Scanner.scan`."""
        return Span[UInt8, __lifetime_of(self)](unsafe_ptr=self.token.unsafe_ptr(), len=self.token.size)

    fn current_token_as_string_slice(ref [_]self) -> StringSlice[__lifetime_of(self)]:
        """Returns the most recent token generated by a call to `Scanner.scan`."""
        return StringSlice[__lifetime_of(self)](unsafe_from_utf8_ptr=self.token.unsafe_ptr(), len=self.token.size)
    ...
    fn scan(inout self) -> Bool:
        ...
        `Scanner.current_token_as_bytes_span`, and `Scanner.current_token_as_string_slice` methods.
        ...
        while True:
            ...
            if (self.end > self.start) or self.err:
                ...
                if self.err:
                    at_eof = True
                advance, token, err = split(self.as_bytes_span()[self.start : self.end], at_eof)
                if err:
