...
#                 if self.err:
#                     at_eof = True
#                 advance, token, err = split(self.as_bytes()[self.start : self.end], at_eof)
#                 if err:
...
# alias SplitFunction = fn (data: Span[Byte], at_eof: Bool) -> (
#     Int,
#     List[Byte, True],
#     Error,
# )
...
# ###### split functions ######
# fn scan_bytes(data: Span[Byte], at_eof: Bool) -> (Int, List[Byte, True], Error):
...
#         return 0, List[Byte, True](), Error()

#     return 1, List[Byte, True](data[0:1]), Error()


# fn scan_runes(data: Span[Byte], at_eof: Bool) -> (Int, List[Byte, True], Error):
...
#     var lhs = (((UnsafePointer[Scalar[DType.Byte]].load(data.unsafe_ptr()) >> 7) == 0) * 1).cast[DType.Byte]()
#     var rhs = count_leading_zeros(~UnsafePointer[Scalar[DType.Byte]].load(data.unsafe_ptr()))
#     var char_length = int(lhs + rhs)

#     # Copy N bytes into new pointer and construct List.
#     var sp = UnsafePointer[Byte].alloc(char_length)
#     parallel_memcpy(sp, data.unsafe_ptr(), char_length)
#     var result = List[Byte, True](unsafe_pointer=sp, size=char_length, capacity=char_length)
...
# fn scan_lines(data: Span[Byte], at_eof: Bool) -> (Int, List[Byte, True], Error):
...
# fn scan_words(data: Span[Byte], at_eof: Bool) -> (Int, List[Byte, True], Error):
...
#         width = len(data[i])
#         if is_space(data[i]):
#             return i + width, List[Byte, True](data[start:i]), Error()

#         i += width

#     # If we're at EOF, we have a final, non-empty, non-terminated word. Return it.
#     if at_eof and len(data) > start:
#         return len(data), List[Byte, True](data[start:]), Error()

#     # Request more data.
#     return start, List[Byte, True](), Error()
