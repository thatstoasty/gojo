...
struct Socket:
    ...
    async fn connect(inout self, address: String, port: Int) raises:
    ...
    @always_inline
    async fn write(inout self: Self, src: List[Byte]) -> (Int, Error):
    ...
    async fn send_all(self, src: List[Byte], max_attempts: Int = 3) raises:
        """Send data to the socket. The socket must be connected to a remote socket.
    ...
    fn send_to(inout self, src: List[Byte], address: String, port: Int) raises -> Int:
        ...
        var err: Error
        bytes_written, err = await self.write(Span(src))
        if err:
            raise err
        return bytes_written

    @always_inline
    async fn read(inout self, inout dest: List[Byte]) -> (Int, Error):
    ...
    fn send_file(self, file: FileHandle, offset: Int = 0) raises:
        await self.send_all(file.read_bytes())
