...
fn new_response_transfer_write[RWC: io.ReadWriteCloser](response: Arc[Response[RWC]]) raises -> Arc[TransferWriter[io.Reader, io.Closer]]:
    return TransferWriter[io.Reader, io.Closer](TransferWriter[io.Reader, io.Closer](
        True, "", nil, nil, False)
        )
...
struct TransferWriter[R: io.Reader, C: io.Closer]():
    ...
    fn write_header[W: io.Writer](inout self, inout writer: W) -> Optional[WrappedError]:
        if self.close:
            ...
            if not contains_token:
                var result = io.write_string[W](writer, "Connection: close\r\n")
                if result.error:
                    return result.error
        ...
        if True:
            var result = io.write_string[W](writer, "Content-Length: ")
            if result.error:
                return result.error

            result = io.write_string[W](writer, self.content_length)
            if result.error:
                return result.error

        return None

    fn write_body[W: io.Writer](inout self, writer: W) -> Optional[WrappedError]:
        var ncopy int64
        closed := false
        defer fn():
