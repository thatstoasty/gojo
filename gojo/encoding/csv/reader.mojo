...
# fn (e *ParseError) Unwrap() error: return e.Err 
...
alias ERR_INVALID_DELIM = "csv: invalid field or comment delimiter"

# TODO: No rune support for now
fn valid_delim(delim: String) -> Bool:
	return delim != '"' and delim != '\r' and delim != '\n'
...
# fn NewReader(r io.Reader) *Reader:
# 	return &Reader{
# 		Comma: ',',
# 		r:     bufio.NewReader(r),
...
# fn (r *Reader) Read() (record []string, err error):
# 	if r.ReuseRecord:
...
# fn (r *Reader) FieldPos(field int) (line, column int):
# 	if field < 0 or field >= len(r.fieldPositions):
...
# # reported.
# fn (r *Reader) ReadAll() (records [][]string, err error):
# 	for:
# 		record, err := r.readRecord(nil)
# 		if err == io.EOF:
# 			return records, nil
...
# fn (r *Reader) readLine() ([]byte, error):
...
# 	# Normalize \r\n to \n on all input lines.
# 	if n := len(line); n >= 2 and line[n-2] == '\r' and line[n-1] == '\n':
# 		line[n-2] = '\n'
# 		line = line[:n-1]
...
# fn (r *Reader) readRecord(dst []string) ([]string, error):
# 	if r.Comma == r.Comment or !validDelim(r.Comma) or (r.Comment != 0 and !validDelim(r.Comment)):
...
# 		if r.Comment != 0 and nextRune(line) == r.Comment:
# 			line = nil
# 			continue # Skip comment lines
		
# 		if errRead == nil and len(line) == lengthNL(line):
# 			line = nil
...
# 			i := bytes.Indexfn(line, fn(r rune) bool:
...
# 					if !r.LazyQuotes and errRead == nil:
...
# 		if len(dst) != r.FieldsPerRecord and err == nil:
# 			err = &ParseError{
# 				StartLine: recLine,
# 				Line:      recLine,
# 				Column:    1,
