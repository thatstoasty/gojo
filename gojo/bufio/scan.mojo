...
struct Scanner[R: io.Reader]():
    ...
    fn scan(inout self) raises -> Bool:
        ...
        while True:
            ...
            if (self.end > self.start) or self.err:
                ...
                if self.err:
                    at_eof = True
                advance = self.split(self.buf[self.start:self.end], at_eof, token, err)
                if err:
                    if str(err.value()) == ErrFinalToken:
                        self.token = token
                        self.done = True
                ...
                if len(token) != 0:
                    if not self.err or advance > 0:
                        ...
                        if self.empties > MAX_CONSECUTIVE_EMPTY_READS:
                            raise Error("bufio.Scan: too many empty tokens without progressing")
            ...
            while True:
                ...
                if loop > MAX_CONSECUTIVE_EMPTY_READS:
                    self.set_err(Err(Error(io.ErrNoProgress)))
                    break
    ...
    fn advance(inout self, n: Int) -> Bool:
        if n < 0:
            self.set_err(Err(Error(ErrNegativeAdvance)))
            return False

        if n > self.end-self.start:
            self.set_err(Err(Error(ErrAdvanceTooFar)))
            return False
    ...
    # fn split(inout self, split_function: split_fn) raises:
    #     if self.scan_called:
...
# TODO: For now, passing in token and err to be modified by the split_fn. This is because the Tuple return unpacking cannot handle a memory only type (Bytes and Err)
alias split_fn = fn(data: Bytes, at_eof: Bool, inout token: Bytes, inout err: Err) raises -> Int
...
alias ErrFinalToken = "final token"
...
fn scan_lines(data: Bytes, at_eof: Bool, inout token: Bytes, inout err: Err) raises -> Int:
    if at_eof and len(data) == 0:
        return 0
...
# scan_words is a split fntion for a [Scanner] that returns each
# space-separated word of text, with surrounding spaces deleted. It will
# never return an empty string. The definition of space is set by
# unicode.IsSpace.
# TODO: Handle runes and utf8 decoding. For now, just assuming single byte length. 
fn scan_words(data: Bytes, at_eof: Bool, inout token: Bytes, inout err: Err) raises -> Int:
    # Skip leading spaces.
    var start = 0
    var width = 0
